...
//const int16_t HEIGHT = 64;   // This is the 'raw' display w/h - never changes
extern __IO uint16_t WIDTH;
extern __IO uint16_t HEIGHT;

int16_t     _width, _height, // Display w/h as modified by current rotation
            cursor_x, cursor_y;
uint16_t    textcolor, textbgcolor;
uint8_t     textsize, rotation;
int8_t      wrap,   // If set, 'wrap' text at right edge of display
            _cp437; // If set, use correct CP437 charset (default is off)
GFXfont
  *gfxFont;


void ARM_GFX(int16_t w, int16_t h){
    _width    = WIDTH;
    _height   = HEIGHT;
    rotation  = 0;
    cursor_y  = cursor_x    = 0;
...
// pos.  Specifically for 8-bit display devices such as IS31FL3731;
// no color reduction/expansion is performed.
void drawGrayscaleBitmap_(int16_t x,
    int16_t y,
    uint8_t *bitmap,
    int16_t w,
    int16_t h) {
...
// no color reduction/expansion is performed.
void drawGrayscaleBitmap__(int16_t x,
    int16_t y,
    const uint8_t bitmap[],
    const uint8_t mask[],
    int16_t w,
...
// no color reduction/expansion is performed.
void drawGrayscaleBitmap___(int16_t x,
    int16_t y,
    uint8_t *bitmap,
    uint8_t *mask,
    int16_t w,
...
void drawRGBBitmap__(int16_t x,
    int16_t y,
    const uint16_t bitmap[],
    const uint8_t mask[],
    int16_t w,
...
// For 16-bit display devices; no color reduction performed.
void drawRGBBitmap___(int16_t x,
    int16_t y,
    uint16_t *bitmap,
    uint8_t *mask,
    int16_t w,
